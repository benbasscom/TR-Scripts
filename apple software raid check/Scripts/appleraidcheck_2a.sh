...
# 0.9.5-a Designed for 2 apple software RAIDS.
...
sw_raid_name_1="$(echo "$sw_disk_raw" | grep "Name:" | cut -d : -f2 | cut -c 18- | head -1)"
sw_raid_name_2="$(echo "$sw_disk_raw" | grep "Name:" | cut -d : -f2 | cut -c 18- | tail -1)"

sw_raid_1_member1_status="$(echo "$sw_disk_raw" | grep "0  disk" | awk '{print $4}' | head -1)"
sw_raid_2_member1_status="$(echo "$sw_disk_raw" | grep "0  disk" | awk '{print $4}' | tail -1)"

sw_raid_1_member2_status="$(echo "$sw_disk_raw" | grep "1  disk" | awk '{print $4}' | head -1)"
sw_raid_2_member2_status="$(echo "$sw_disk_raw" | grep "1  disk" | awk '{print $4}' | tail -1)"

sw_raid_1_member1_disk="$(echo "$sw_disk_raw" | grep "0  disk" | awk '{print $2}' | head -1)"
sw_raid_2_member1_disk="$(echo "$sw_disk_raw" | grep "0  disk" | awk '{print $2}' | tail -1)"

sw_raid_1_member2_disk="$(echo "$sw_disk_raw" | grep "1  disk" | awk '{print $2}' | head -1)"
sw_raid_2_member2_disk="$(echo "$sw_disk_raw" | grep "1  disk" | awk '{print $2}' | tail -1)"
...
sw_smart_check_raw="$(system_profiler SPSerialATADataType)"
	...
	"Date:"'\t''\t''\t'"$(date +%m/%d/%Y)"'\n'		\
	"Time:"'\t''\t''\t'"$(date +%H:%M)"'\n'		\
	"Raid Name:"'\t''\t'"$sw_raid_name_1"'\n'		\
	"Raid Status:"'\t''\t'"$sw_raid_status_1"'\n''\n'			\
	"Member 1 Status:"'\t'"$sw_raid_1_member1_status_1"'\n'			\
	"Member 1 Disk:"'\t''\t'"$sw_raid_1_member1_disk"'\n'			\
	"Member 1 Identifier:"'\t'"$sw_smart_dev_name_1"'\n'			\
	"Member 2 Status:"'\t'"$sw_raid_1_member2_status"'\n'			\
	"Member 2 Disk:"'\t''\t'"$sw_raid_1_member2_disk"'\n'			\
...
sw_smart_check_raw="$(system_profiler SPSerialATADataType)"
	...
	"Date:"'\t''\t''\t'"$(date +%m/%d/%Y)"'\n'		\
	"Time:"'\t''\t''\t'"$(date +%H:%M)"'\n'		\
	"Raid Name:"'\t''\t'"$sw_raid_name_2"'\n'		\
	"Raid Status:"'\t''\t'"$sw_raid_status_2"'\n''\n'			\
	"Member 1 Status:"'\t'"$sw_raid_2_member1_status"'\n'			\
	"Member 1 Disk:"'\t''\t'"$sw_raid_2_member1_disk"'\n'			\
	"Member 1 Identifier:"'\t'"$sw_smart_dev_name_1"'\n'			\
	"Member 2 Status:"'\t'"$sw_raid_2_member2_status"'\n'			\
	"Member 2 Disk:"'\t''\t'"$sw_raid_2_member2_disk"'\n'			\
	"Member 2 Identifier:"'\t'"$sw_smart_dev_name_2"'\n'			\
	| mail -s ""$host_name" Apple Raid Failure Notice" "$to"; \
